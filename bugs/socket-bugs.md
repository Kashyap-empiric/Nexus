...
### 4. HTTP Rate Limiter Map Never Cleaned Up — Memory Leak

**File:** `server/src/middlewares/rateLimiter.ts:16`

```typescript
const hits = new Map<string, { count: number; resetAt: number }>();
```

Same pattern as Bug 2 above. The `hits` Map for HTTP rate limiting is shared across all routes and grows unboundedly. Entries for IPs that stop sending requests are never removed. Every unique IP that has ever made a request leaves a permanent entry. On a public-facing server, this is thousands of entries per day, accumulating indefinitely.

**Also affects:** `server/src/socket/middlewares/rateLimiter.ts:4` — the `messageRateLimits` Map has the same issue (already documented as Bug 2).

### 5. Rate Limiter Is In-Memory — Broken in Multi-Instance Deployments

**Files:**
- `server/src/middlewares/rateLimiter.ts:16` — in-memory `Map`
- `server/src/socket/middlewares/rateLimiter.ts:4` — in-memory `Map`

Both rate limiters use in-memory Maps. On Render (or any platform with multiple replicas):
- Each instance has its own independent counter
- A user can send 20 messages to instance A, wait, send 20 more to instance B — doubling the effective limit
- Rate limiting provides no protection against distributed abuse

### 6. Redis Reconnection Strategy Gives Up After 1 Retry

**File:** `server/src/lib/redis.ts:7-9`

```typescript
if (retries >= 1) return false;
```

Redis only retries the initial connection once. If the Redis server is temporarily unavailable (e.g., during a Render restart or network blip):
- The `retryStrategy` returns `false` on the second attempt
- The `redisAvailable` getter in `presenceStore.ts:19-25` returns `false`
- Presence tracking falls back to in-memory only
- **It never retries again** — the process must be restarted to re-enable Redis

A better approach: retry with exponential backoff for several minutes before giving up.

---

## MEDIUM BUGS

### 7. `async` Keyword Unused in `registerPresenceHandlers`

**File:** `server/src/socket/handlers/presence.handler.ts:7`

```typescript
export const registerPresenceHandlers = async (io: Server, socket: Socket) => {
```

The function is declared `async` but is called **without `await`** in `socket.ts:75`:
```typescript
registerPresenceHandlers(io, socket);  // no await
registerMessageHandlers(socket);
registerWorkspaceHandlers(io, socket);
```

The only `await` inside the function is `await presenceStore.addSocket(...)` on line 33. But because the caller doesn't `await` the function, the `connect` event handler returns before the presence state is fully initialized. If `workspace:join` or `message:send` events fire immediately after connect (before presence init completes), they operate on stale or incomplete presence state.

**Fix:** Either add `await` at the call site, or remove `async` and use the synchronous fallback (the in-memory store is synchronous).

### 8. `socket.rooms.forEach` Iterates Internal Socket.io Rooms
...
`socket.rooms` is a `Set<string>` that includes **all** rooms the socket belongs to, including Socket.io internal rooms (like the socket's own ID room, e.g., `socket#<id>`). Modifying `socket.rooms` during iteration (via `socket.leave()`) is **undefined behavior** for JavaScript Sets. While Socket.io handles this gracefully in practice, it violates the ECMAScript specification.

Additionally, if a socket ID happens to start with `conversation:` (theoretically possible), it would attempt to leave its own system room.

### 9. Status Updates Not Broadcast to DM Conversation Partners Outside Workspaces

**File:** `server/src/socket/socket.dispatcher.ts:84-106`

```typescript
export const dispatchUserStatusUpdate = (userId: string, status: UserStatus, statusText?: string | null) => {
  getIO().to(`user:${userId}`).emit(SOCKET_EVENTS.USER_STATUS_UPDATE, payload);
  // Also broadcast to workspace rooms
  userWorkspaceIds.forEach((workspaceId) => {
    getIO().to(`workspace:${workspaceId}`).emit(SOCKET_EVENTS.USER_STATUS_UPDATE, payload);
  });
};
```

The status update is broadcast to:
1. The user's own room (`user:${userId}`)
2. All workspace rooms the user belongs to

It is **NOT** broadcast to DM conversation rooms (`conversation:${dmId}`). Two users who have a DM but do NOT share a workspace will never see each other's status changes. The comment on lines 101-102 acknowledges this but doesn't fix it.

### 10. Presence Dispatch Uses `payload as any` — Loses Type Safety

**File:** `server/src/socket/handlers/presence.handler.ts:51`

```typescript
io.emit(SOCKET_EVENTS.USER_ONLINE, payload as any);
```

The `as any` cast silences type mismatches. If the payload shape changes (e.g., adding a required field), the compiler won't flag the emit call. The same pattern appears on line 56 (`io.emit(SOCKET_EVENTS.USER_OFFLINE, payload as any)`).

### 11. Socket Auth Middleware Import Path With Uncertain Comment
...
The comment `// adjust path as needed` indicates the developer was uncertain about the import path. If run from a different working directory or build output structure, this import may silently fail. Other socket files use `.js` extensions in their imports — this one does not.

### 12. PRESENCE INITIAL Payload Shape Mismatch Causes Silent Failure
...
### 13. `useSocketEvents` Has Fragile Handler Identity Tracking
...
### 14. Duplicate Event Processing: Global + Per-Conversation Handlers for Same Events
...
### 15. Workspace Channel Room Names Collide with DM Room Names
...
### 16. `workspace:join` Leaves Previous Conversation Rooms Stale on Room-Name Prefix Collision
...
### 17. `dispatchConversationUpdate` Builds Ad-Hoc Payload Instead of Reusing Dispatchers
...
### 18. `dispatchUserPresence` ONLINE Broadcasts to ALL Clients Without Target Socket
...
### 19. Client and Server Maintain Duplicate `SOCKET_EVENTS` Constants
...
### 20. `conversation:update` Event Used for Two Different Purposes
...
### 21. `handleMessageNew` Document Title Accumulates Multiple Prefixes
...
### 22. `TypingIndicator` Relies on Render Cycle to Expire Stale Entries
...
### 23. `presenceStore.redisAvailable` Can Throw Instead of Returning False
...
### 24. `workspace:join` Doesn't Validate Payload Shape
...
### 25. Socket Auth Middleware Path Resolution May Fail
...
### 26. `useSocketEvents` Depends on Object Reference — Inefficient Re-Registration
...
### 27. `handleConversationNew` Overwrites `updatedAt` Without Latest Message
...
### 28. `socket.auth` Callback May Reconnect with Stale Token After Session Refresh
...
### 29. `SocketProvider` Assumes Socket Connects Immediately
