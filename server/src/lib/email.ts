...
export async function sendPasswordResetEmail(
  ...
  if (!ENV.SENDGRID_API_KEY) {
    console.warn(
      `[email] SENDGRID_API_KEY not configured — would send password reset to ${params.to}`,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `[email] ✓ Password reset sent to=${params.to}  messageId=${messageId}`,
    );
